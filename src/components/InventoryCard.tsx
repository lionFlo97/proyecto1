...
interface InventoryCardProps {
  ...
  onMaterialExit?: (item: InventoryItem) => void;
  isUpdating: boolean;
  isDeleting?: boolean;
  isViewerMode?: boolean;
}

export function InventoryCard({ item, onUpdateStock, onEditItem, onDeleteItem, onReassignCategory, onMaterialExit, isUpdating, isDeleting = false, isViewerMode = false }: InventoryCardProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [newStock, setNewStock] = useState(item.stock);

  const getStockStatus = (stock: number) => {
    const puntoPedido = item.puntoPedido || 5;
    if (stock === 0) return { color: 'text-gray-600', bg: 'bg-gray-100', status: 'Sin Stock' };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-all duration-200">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-start space-x-3">
          <div className={`p-3 rounded-xl ${item.tipo === 'ERSA' ? 'bg-red-100' : 'bg-blue-100'}`}>
            {item.foto ? (
        ...
        <div className={`px-3 py-2 rounded-full text-sm font-medium ${stockStatus.bg} ${stockStatus.color}`}>
          {stockStatus.status}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-1">
            {!isEditing && (
              <button
                ...
                className="flex items-center space-x-1 px-2 py-1 text-xs text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
              >
                <Edit3 className="h-4 w-4" />
                <span>Stock</span>
              </button>
            )}
            {onMaterialExit && item.stock > 0 && (
              <button
                onClick={() => onMaterialExit(item)}
                className="flex items-center space-x-1 px-2 py-1 text-xs text-orange-600 hover:bg-orange-50 rounded-lg transition-colors"
              >
            ...
            {onEditItem && (
              <button
                ...
                className="flex items-center space-x-1 px-2 py-1 text-xs text-green-600 hover:bg-green-50 rounded-lg transition-colors"
              >
            ...
            {onReassignCategory && (
              <button
                ...
                className="flex items-center space-x-1 px-2 py-1 text-xs text-purple-600 hover:bg-purple-50 rounded-lg transition-colors"
            ...
            {onDeleteItem && (
              <button
                ...
                disabled={isDeleting}
                className="flex items-center space-x-1 px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50"
              >
                {isDeleting ? (
        ...
        {isViewerMode && (
          <button
            disabled
            className="flex items-center space-x-1 px-3 py-2 text-sm text-gray-400 bg-gray-50 rounded-lg cursor-not-allowed"
          >
